...
#define bufferSize  2000                                //size for calibration process
...
#pragma NOINIT(monitorBuffer)
int monitorBuffer[900];
...
void calibration(void) {
    ...
    if (calbcount == bufferSize) {
        ...
        flush_wind = flush_wind/2;  //it should be divided by flush
        flush_value = flush_value/2; //it should be divided by flush
        flush_threshold = (flush_value + base)/2;
        base_threshold = base; //was 1.1*base
        current_value = base;
        overflow_maxtime = flush_wind; //was 1.5*flush_wind
        P3OUT = BIT0;
...
void monitor(void){
    ...
    switch (STATE) {
        ...
        case 2:
            ...
            if (current_value < (flush_value + flush_threshold)/2) {
                STATE = 1;
                overflowTime = 0;
                break;
            }
